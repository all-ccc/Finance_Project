...
def get_deposit_products():
    ...
    base_result = json_result['result']['baseList']
    option_result = json_result['result']['optionList']


    result_list = [] # 최종 return값
    # 큰 딕셔너리
    for base in base_result:
        new_list = {}
        new_list['금리정보'] = ''
        new_list['금융상품명'] = base['fin_prdt_nm']
        new_list['금융회사명'] = base['kor_co_nm'] # 딕셔너리 구조 만들기
        ...
        for option in option_result:
            if base['fin_prdt_cd'] == option['fin_prdt_cd']:
                ...
                # 취합된 금리정보를 temp_list의 한 요소에 저장
                temp_list.append(new_dict)
        new_list['금리정보'] = temp_list
